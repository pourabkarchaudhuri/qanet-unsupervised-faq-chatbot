...
class SquadData(object):
    ...
    def load(cls, filepath=None, np_path=None):
        def load_data(prefix):
            ques_ids = np.load(np_path + '/%s_ques_ids.npy' % prefix, allow_pickle=True)
            contextw = np.load(np_path + '/%s_contextw.npy' % prefix, allow_pickle=True)
            queryw = np.load(np_path + '/%s_queryw.npy' % prefix, allow_pickle=True)
            contextc = np.load(np_path + '/%s_contextc.npy' % prefix, allow_pickle=True)
            queryc = np.load(np_path + '/%s_queryc.npy' % prefix, allow_pickle=True)
            p1 = np.load(np_path + '/%s_p1.npy' % prefix, allow_pickle=True)
            p2 = np.load(np_path + '/%s_p2.npy' % prefix, allow_pickle=True)
            start = np.load(np_path + '/%s_start.npy' % prefix, allow_pickle=True)
            end = np.load(np_path + '/%s_end.npy' % prefix, allow_pickle=True)
